...
class TutorialManager {
    constructor() {
        ...
        this.FLAGS = {
            ...
            // アルゴリズムフェーズ
            ALGORITHM_ADVANCED: 'tutorial_step2_algorithm_advanced',
    ...
    autoStart() {
        ...
        // 登録済みチュートリアルをチェック（登録順にチェックするため、順序が重要）
        const priorityOrder = [
            'seikai_create', // 名前入力（最優先）
            'fuseikai_create',
            'mondai_create',
            'seikai',        // システム作成
            'fuseikai',
        ...
        for (const name of priorityOrder) {
            ...
            if (config) {
                if (this.checkTrigger(config.trigger)) {
                    console.log(`🎬 チュートリアル開始 (優先): ${name}`);
                    this.start(name);
                    return;
                }
            }
    ...
    checkTrigger(trigger) {
        ...
        if (trigger.requireFlag) {
            ...
            const hasRequired = flags.every(flag => {
                const has = this.hasFlag(flag);
                if (!has) console.log(`  ❌ 必須フラグ不足: ${flag}`);
                return has;
            });
        ...
        if (trigger.forbidFlag) {
            ...
            const hasForbidden = flags.some(flag => {
                const has = this.hasFlag(flag);
                if (has) console.log(`  ❌ 禁止フラグあり: ${flag}`);
                return has;
            });
        ...
        if (trigger.condition && typeof trigger.condition === 'function') {
            ...
            if (!conditionResult) console.log('  ❌ カスタム条件不一致');
